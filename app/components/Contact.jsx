    ...
    return(
        ...
        <div className={`bg-[#000000ab] absolute z-[999] md:z-[1] transition-[width] duration-700 border-[1px] border-[#ffffff33] flex-col rounded-xl items-center flex backdrop-blur-lg mx-auto mt-16 ${resizeContact ? 'w-11/12 md:w-1/3 h-auto' : 'w-[350px] h-fit xl:h-auto'}`}>
            <div onMouseEnter={()=> setHoverContactNav(true)} onMouseLeave={()=> setHoverContactNav(false)} className="nav hidden lg:flex py-5 text-black w-full pl-5">
                ...
                <button onClick={()=> {setToggleContact(false); setResizeContact(false)}} className="h-4 w-4  flex justify-center items-center rounded-xl mx-1 bg-[#ff6059]"><GrFormClose className={`${hoverContactNav? 'block' : 'hidden'} text-sm`}/></button>
                <div onClick={()=> setToggleContact(false)} className="h-4 w-4  rounded-xl mx-1 bg-[#ffbd2e]"><MdMinimize className={`${hoverContactNav? 'block' : 'hidden'} -mt-0.5`}/></div>
                <div onClick={()=> setResizeContact(!resizeContact)} className="h-4 w-4  flex justify-center items-center rounded-xl mx-1 bg-[#28c941]"><IoMdResize className={`${hoverContactNav? 'block' : 'hidden'} text-xs`}/></div>
            </div>
            <div onMouseEnter={()=> setHoverContactNav(true)} onMouseLeave={()=> setHoverContactNav(false)} className="lg:hidden flex py-5 text-black w-full pl-5">
                ...
                <button onClick={()=> {setToggleContact(false); console.log('gg'); setResizeContact(false)}} className="h-4 w-4  flex justify-center items-center rounded-xl mx-1 bg-[#ff6059]"><GrFormClose className={`${hoverContactNav? 'block' : 'hidden'} text-sm`}/></button>
                <div onClick={()=> setToggleContact(false)} className="h-4 w-4  rounded-xl mx-1 bg-[#ffbd2e]"><MdMinimize className={`${hoverContactNav? 'block' : 'hidden'} -mt-0.5`}/></div>
                <div onClick={()=> setResizeContact(!resizeContact)} className="h-4 w-4  flex justify-center items-center rounded-xl mx-1 bg-[#28c941]"><IoMdResize className={`${hoverContactNav? 'block' : 'hidden'} text-xs`}/></div>
            </div>
